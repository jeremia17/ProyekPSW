<section class="mt-10">
    ...
    <form wire:submit.prevent="addToCart">
            ...
            <div class="grid grid-cols-4 gap-4">
                    @foreach($locations as $location)
                            <label wire:key="location-{{ $location->id }}" wire
                                ...
                                x-bind:class="{
                                'bg-yellow-500 text-white ': selectedLocation === {{ $location->id }},
                                'bg-gray-50 hover:bg-yellow-100': selectedLocation !== {{ $location->id }}
                            }"
                            >
                ...
                <div class="grid grid-cols-3 gap-4" x-data="{ selectedTimeSlot : @entangle('selectedTimeSlot').defer }">
                    @foreach($timeSlots as $timeSlot)
                        ...
                        <label
                            ...
                            class="group relative flex items-center text-gray-800 justify-center rounded-md border py-3 px-4 text-sm font-medium uppercase focus:outline-none sm:flex-1 cursor-pointer shadow-sm"
                            x-bind:class="{
                                'bg-yellow-500 text-white ': selectedTimeSlot === {{ $timeSlot->id }},
                                'bg-gray-50 hover:bg-yellow-100': selectedTimeSlot !== {{ $timeSlot->id }},
                            }"
                        >
                            <input type="radio" name="time-slot-choice"
        ...
        <button type="submit"
                class="mt-6 flex w-full items-center justify-center rounded-md border border-transparent bg-yellow-600 px-8 py-3 text-base font-medium text-white hover:bg-yellow-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2">
            Add to cart
        </button>
    </form>
</section>
